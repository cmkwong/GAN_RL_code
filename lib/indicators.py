...
class Bollinger_Bands:
    ...
    def normalise_one(self, offset, train_mode):
        if train_mode is False:
            offset = offset + self.cutoff
        target_data = (self.target_price['close'][offset] - self.lowerBand[offset]) / \
                      (self.upperBand[offset] - self.lowerBand[offset])
        return target_data
...
class MACD:
    ...
    def normalise_one(self, offset, train_mode):
        if train_mode is False:
            offset = offset + self.cutoff
        target_data = np.ndarray(shape=(1, self.encoded_size), dtype=np.float64)
        target_data[0, 0] = self.MACD_fast_n[offset]
        target_data[0, 1] = self.MACD_slow_n[offset]
        return target_data.reshape(-1, self.encoded_size)
...
class RSI:
    ...
    def normalise_one(self, offset, train_mode):
        if train_mode is False:
            offset = offset + self.cutoff
        target_data = self.rsi_value[offset] / 100
        return target_data.reshape(-1, self.encoded_size)
